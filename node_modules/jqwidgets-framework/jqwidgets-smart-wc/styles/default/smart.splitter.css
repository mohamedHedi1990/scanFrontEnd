...
smart-splitter {
    ...
    position: relative; /* Fixes the issue with the overflow and resize-container */
}
...
.smart-splitter .smart-splitter-resize-button > span {
    width: 6px;
    height: 6px;
    border: 1px solid var(--smart-background-color);
...
.smart-splitter smart-splitter-bar.smart-element {
    background: var(--smart-ui-state-border-focus);
